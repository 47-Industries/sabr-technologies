...
export class NeuralDust {
    constructor(scene, count = 25000) {
        ...
        // Keep the originals + a greyed copy so the particles desaturate to
        // grey/white when Leon sleeps — a glanceable "he's under" signal.
        this._origColors = new Float32Array(colors);
        this._greyColors = new Float32Array(count * 3).fill(0.62);
        this._asleep = false;
    ...
    update(time, activity = 0.1) {
        ...
        // Sleep mode — desaturate to grey and go still + dim so it's obvious at
        // a glance that Leon is asleep. Driven by the dashboard sleep poller.
        const asleep = !!(typeof window !== 'undefined' && window.__leonAsleep);
        if (asleep !== this._asleep) {
            this._asleep = asleep;
            const colAttr = this.mesh.geometry.attributes.color;
            colAttr.array.set(asleep ? this._greyColors : this._origColors);
            colAttr.needsUpdate = true;
        }

        const speed = asleep ? 0.12 : (0.3 + activity * 2.5);
        ...
        // Pulse with activity — but go dim and still while Leon sleeps.
        const eff = asleep ? 0.03 : activity;
        this.mesh.material.opacity = (asleep ? 0.06 : 0.1) + eff * 0.4;
        this.mesh.material.size = 0.004 + eff * 0.01;
    }
}
